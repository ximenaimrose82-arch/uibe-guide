...
console.log(`数据完整性: FAQ=${FAQ.length} 条, GUIDES=${GUIDES.length} 篇, 问题数=${problems}`);

// ---- 2. 加载 index.html 的主脚本(最后一个无 src 的 <script>) ----
const html = readFileSync(join(root, 'index.html'), 'utf8');
const inlineScripts = [...html.matchAll(/<script>([\s\S]*?)<\/script>/g)].map(m => m[1]);
const js = inlineScripts[inlineScripts.length - 1] + '\n;return { esc, highlight, countdownText, daysUntil, matchFaq, matchGuide, msgHTML, ALL_KEYS };';
